...
try:
    ...
    if authtuple:
        ...
        githubauth = base64.encodestring(
            auth_string).decode().replace('\n', '')
    else:
        githubauth = None
except:
    githubauth = None
...
if not depsonly:
    ...
    githubreq = urllib.request.Request(
        "https://api.github.com/search/repositories?q=%s+user:chitangUI+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
                ...
                if 'branch' not in dependency:
                    try:
                        dependency['branch'] = get_default_or_fallback_revision(
                            'chitangUI/' + dependency['repository'])
                        dependency['repository'] = 'chitangUI/' + \
                            dependency['repository']
                        print("%s is resolved from chitangUI" %
                              (dependency['repository']))
                    except:
                        dependency['branch'] = get_default_or_fallback_revision(
                            'LineageOS/' + dependency['repository'])
                        dependency['repository'] = 'LineageOS/' + \
                            dependency['repository']
                        print("%s is resolved from LineageOS" %
                              (dependency['repository']))
...
def get_default_or_fallback_revision(repo_name):
    ...
    if os.getenv('ROOMSERVICE_BRANCHES'):
        fallbacks += list(filter(bool,
                          os.getenv('ROOMSERVICE_BRANCHES').split(' ')))
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if re.match(r"^android_device_[^_]*_" + device + "$", repo_name):
            ...
            manufacturer = repo_name.replace(
                "android_device_", "").replace("_" + device, "")
            repo_path = "device/%s/%s" % (manufacturer, device)
            revision = get_default_or_fallback_revision(
                'chitangUI/' + repo_name)

            device_repository = {'repository': 'chitangUI/' +
                                 repo_name, 'target_path': repo_path, 'branch': revision}
